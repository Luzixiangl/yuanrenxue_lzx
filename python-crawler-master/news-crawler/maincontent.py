...
REGEXES = {
    'positiveRe': re.compile(                                                       # 一般出现在正文部分的class和id中的关键词
    ...
    'negativeRe': re.compile(                                                       # 一般出现在非正文的class和id中的关键词
...
class MainContent:
    def __init__(self,):
        self.non_content_tag = set([        # 不会包含正文的标签，如遇到此类标签节点，直接忽略
            'head',
            'meta',
            'script',
            'style',
    ...
    def get_title(self, doc):
        ...
        if len(title) < 7:
            tt = doc.xpath('//meta[@name="title"]')     # 尝试从meta里找title
            if tt:
                title = tt[0].get('content', '')
        if len(title) < 7:
            tt = doc.xpath('//*[contains(@id, "title") or contains(@class, "title")]')      # 尝试从body里找id和class包含title的节点
            if not tt:
